...
interface FollowMutateType {
	follow_user: string;
	user_image: string;
}

export default function PublicUserProfile({ user, avatar }: { user: string; avatar: string }) {
	...
	const { mutateAsync: followMutate } = useMutation({
		mutationFn: (follow: FollowMutateType) =>
			followUser({ uri: "/user/follow", method: "POST", body: follow }),
		onError: (error) => {
			dispatch(activate());
