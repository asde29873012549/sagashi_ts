...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	try {
		...
		if (fileType && !allowFileType.includes(fileType.mime)) {
		...
		const cropInfo = fields.cropInfo && JSON.parse(fields.cropInfo[0]).cropInfo;
		...
		if (
			...
			"code" in err &&
			err.code === formidableError.biggerThanMaxFileSize
		) {
			res
				.status(500)
